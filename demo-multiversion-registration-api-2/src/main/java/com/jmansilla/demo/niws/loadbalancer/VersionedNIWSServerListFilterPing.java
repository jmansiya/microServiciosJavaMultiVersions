...
public class VersionedNIWSServerListFilterPing<T extends Server> extends DefaultNIWSServerListFilter<T> {
	...
	private final SpringBootPing iPingImplement = new SpringBootPing();
	
	public VersionedNIWSServerListFilterPing(org.springframework.cloud.client.discovery.DiscoveryClient _discoveryClient,
	 ...
	 public List<T> getFilteredListOfServers(List<T> servers) {
		 ...
		 if (result != null && result.size() > 0 ){
			 for (T server : result){
				 if (!iPingImplement.isAlive(server)){
...
//					System.out.println("HOST: " + server.getHost() + " PORT: " + server.getPort() + " No está activo.");
//					System.out.println("ERRORR    ====================== ");
//				}
